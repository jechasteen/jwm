...
/* key definitions */
#define MODKEY Mod4Mask     // dwm's default is Mod1Mask aka alt
...
applyrules(Client *c)
{
	...
	c_class    = ch.res_class ? ch.res_class : broken;
	instance = ch.res_name  ? ch.res_name  : broken;

	for (i = 0; i < LENGTH(rules); i++) {
		r = &rules[i];
		if ((!r->title || strstr(c->name, r->title))
		&& (!r->c_class || strstr(c_class, r->c_class))
...
// TODO: runautostart should be more robust
void
runautostart(void) {
    system("cd ~/.config/jwm; ./autostart.sh &");
}
